...
import {
  ArrowLeft, Eye, Plus, GripVertical, Trash2, Type, Hash, AlignLeft,
  CheckSquare, List, Calendar, Mail, ChevronDown, Star, ToggleLeft,
  Settings, Palette, X, Copy, ChevronRight, Layers, Sparkles,
  LayoutTemplate, Monitor, Check, Zap,
} from "lucide-react";
...
const FIELD_TYPES = [
  { type: "short_text",    label: "Short Text",    icon: Type,        color: "text-blue-400",   bg: "bg-blue-400/10" },
  { type: "long_text",     label: "Long Text",     icon: AlignLeft,   color: "text-purple-400", bg: "bg-purple-400/10" },
  ...
  { type: "single_select", label: "Single Select", icon: ChevronDown, color: "text-cyan-400",   bg: "bg-cyan-400/10" },
  { type: "multi_select",  label: "Multi Select",  icon: CheckSquare, color: "text-pink-400",   bg: "bg-pink-400/10" },
  { type: "dropdown",      label: "Dropdown",      icon: List,        color: "text-yellow-400", bg: "bg-yellow-400/10" },
  { type: "checkbox",      label: "Checkbox",      icon: ToggleLeft,  color: "text-emerald-400",bg: "bg-emerald-400/10" },
...
const THEMES = [
  {
    ...
    preview: { bg: "bg-white/10 backdrop-blur", accent: "#6366f1", text: "#1e1e2e", border: "border-white/30" },
  ...
  },
  {
    ...
    preview: { bg: "bg-violet-950", accent: "#a855f7", text: "#f0abfc", border: "border-purple-500/50" },
  ...
  switch (themeId) {
    ...
    case "anime-neon":
      return {
        ...
        input: "bg-purple-900/50 border-purple-500/50 text-purple-100 placeholder:text-purple-400/50",
        label: "text-pink-400 text-sm font-medium",
        button: "bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-500 hover:to-fuchsia-500 text-white",
        title: "text-white font-bold bg-gradient-to-r from-purple-200 to-pink-200 bg-clip-text text-transparent",
...
function PreviewField({ field, themeId }: { field: any; themeId: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {field.type === "date" && (
        <input type="date" className={`w-full px-3 py-2 rounded-lg border text-sm outline-none ${s.input}`} readOnly />
      )}
      {field.type === "single_select" && (
        <div className="flex flex-wrap gap-2">
          {options.length > 0 ? options.map((opt: string) => (
            <button key={opt} className={`px-3 py-1.5 rounded-full border text-sm ${s.input}`}>{opt}</button>
...
function SortableFieldItem({
  field, isSelected, onSelect, onDelete, onDuplicate, themeIcon,
}: {
  field: any;
  isSelected: boolean;
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {/* Field icon */}
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${ft?.bg ?? "bg-white/10"}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className={`text-xs font-medium uppercase tracking-wider ${ft?.color ?? "text-white/50"}`}>
              {ft?.label ?? field.type}
            </span>
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5 min-h-[32px]">
        {options.map((opt) => (
          <span key={opt} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/10 border border-white/10 text-xs text-white/80">
            {opt}
            <button onClick={() => onChange(options.filter((o) => o !== opt))} className="ml-0.5 hover:text-red-400 transition-colors">
              <X className="w-3 h-3" />
            </button>
          </span>
        ))}
      ...
      <div className="flex gap-2">
        ...
        />
        <Button size="sm" onClick={add} className="bg-white/10 hover:bg-white/20 text-white border-0 h-8 px-3">
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-[#0a0a0a] text-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-56 flex-shrink-0 border-r border-white/8 bg-black/40 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
            {FIELD_TYPES.map((ft) => (
              <motion.button
                ...
                className="w-full group flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-white/8 transition-all text-left"
              >
                <div className={`w-7 h-7 rounded-md flex items-center justify-center flex-shrink-0 ${ft.bg} transition-colors`}>
                  <ft.icon className={`w-3.5 h-3.5 ${ft.color}`} />
                </div>
                <span className="text-xs font-medium text-white/70 group-hover:text-white/90 transition-colors">{ft.label}</span>
          ...
          <div className="px-4 py-3 border-t border-white/8">
            <p className="text-xs text-white/30 text-center">{fields.length} field{fields.length !== 1 ? "s" : ""}</p>
          </div>
        </aside>

        {/* ── Center — Canvas ── */}
        <main className="flex-1 overflow-y-auto bg-[#0d0d0d]" onClick={() => setSelectedFieldId(null)}>
          {/* Form header preview */}
          <div className="max-w-2xl mx-auto pt-10 px-6 pb-4">
                      ...
                      {fields.map((field) => (
                        ...
                        >
                          <SortableFieldItem
                            ...
                            onSelect={(id) => { setSelectedFieldId(id); setRightTab("properties"); }}
            ...
            {fields.length > 0 && (
              <div className="mt-4 flex justify-center">
                <button
                  onClick={(e) => { e.stopPropagation(); handleAddField("short_text"); }}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      return (
                        <>
                          <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${ft?.bg ?? "bg-white/10"}`}>
                            <Icon className={`w-3.5 h-3.5 ${ft?.color ?? "text-white/60"}`} />
                          </div>
                          <div>
                            <p className="text-xs font-semibold text-white/80">{ft?.label ?? localField.type}</p>
                  ...
                  {!["checkbox", "rating", "single_select", "multi_select"].includes(localField.type) && (
                    <div className="space-y-1.5">
                  ...
                  {/* Helper Text */}
                  <div className="space-y-1.5">
                    <Label className="text-xs text-white/50 uppercase tracking-wider">Helper Text</Label>
                    <Textarea
                  ...
                  {["single_select", "multi_select", "dropdown", "checkbox"].includes(localField.type) && (
                    <div className="space-y-1.5">
                  ...
                  {localField.type === "number" && (
                    <div className="space-y-3">
                      <Label className="text-xs text-white/50 uppercase tracking-wider">Validations</Label>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) => saveFieldEdit({ validations: { ...localField.validations, min: e.target.value } })}
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) => saveFieldEdit({ validations: { ...localField.validations, max: e.target.value } })}
                            className="bg-white/5 border-white/10 text-white text-xs h-7"
                            placeholder="100"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) => saveFieldEdit({ validations: { ...localField.validations, minLength: e.target.value } })}
                            className="bg-white/5 border-white/10 text-white text-xs h-7"
                            placeholder="0"
                          />
                        </div>
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) => saveFieldEdit({ validations: { ...localField.validations, maxLength: e.target.value } })}
                            className="bg-white/5 border-white/10 text-white text-xs h-7"
                            placeholder="500"
                          />
                        </div>
            ...
            <TabsContent value="theme" className="flex-1 overflow-y-auto m-0 p-4">
              <div className="space-y-3">
                <div className="space-y-0.5 mb-4">
                  ...
                  <p className="text-xs text-white/30">Choose how your form looks to respondents</p>
                </div>
                ...
                {THEMES.map((theme) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`h-14 bg-gradient-to-br ${theme.gradient} flex items-center justify-center relative overflow-hidden`}>
                        ...
                        <div className="absolute right-3 top-2 bottom-2 w-16 rounded-md overflow-hidden opacity-70">
                          <div className={`h-full flex flex-col gap-1 p-1.5 ${theme.gradient}`}>
                            <div className={`h-1.5 w-10 rounded-full ${theme.accentColor} opacity-80`} />
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-2xl w-full max-h-[90vh] p-0 overflow-hidden bg-zinc-950 border-white/10">
          ...
          {/* Preview content */}
          <div className={`overflow-y-auto max-h-[calc(90vh-56px)] ${getThemeStyles(form?.theme ?? "apple-glass").wrapper}`}>
            <div className="max-w-lg mx-auto p-8">
              {/* Form header */}
              <div className="mb-8">
                <h1 className={`text-2xl font-bold mb-2 ${getThemeStyles(form?.theme ?? "apple-glass").title}`}>
                  {form.title}
                </h1>
                {form.description && (
                  <p className={`text-sm ${getThemeStyles(form?.theme ?? "apple-glass").desc}`}>{form.description}</p>
                )}
              </div>

              {fields.length === 0 ? (
                <div className={`text-center py-12 ${getThemeStyles(form?.theme ?? "apple-glass").desc}`}>
                ...
                <div className="space-y-6">
                  {fields.map((field) => (
                    <div key={field.id} className={`p-5 rounded-xl ${getThemeStyles(form?.theme ?? "apple-glass").card}`}>
                      <PreviewField field={field} themeId={form?.theme ?? "apple-glass"} />
                    </div>
                  ))}

                  {/* Submit button */}
                  <button className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${getThemeStyles(form?.theme ?? "apple-glass").button}`}>
                    Submit
                  </button>
                </div>
              )}
